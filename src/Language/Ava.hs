...
instance (PP.Pretty v, PP.Pretty u, PP.Pretty c) =>
  ...
  pretty = \case
    ...
    Embed e -> PP.sep [ PP.lbrace, PP.pretty e |> PP.nest 1, PP.rbrace ]
    Pi x typeX b -> PP.vcat
      [ PP.sep [ PP.parens (PP.sep [var x, PP.colon, PP.pretty typeX]), "->" ]
      , PP.pretty b |> id
      ]
    Lam x b -> PP.vcat
      [ PP.hcat [ PP.backslash, var x, PP.space, "->" ]
      , PP.pretty b |> id
      ]
    Ref r -> case r of
    ...
    where
      ...
      parensWhenAnn e = case e of
        _ ::: _ -> PP.pretty e |> PP.indent 1 |> vparens
        _ -> PP.pretty e
      ...
      parensWhenBinder e = case e of
        ...
        Lam _ _ -> ps
        _ -> PP.pretty e
        where ps = PP.pretty e |> PP.indent 1 |> vparens
...
iComb :: AST Zero Name Star () Elim
iComb = expr ::: typ
  where
    expr :: AST Zero Name Star () Term
    expr = lam "a" $ lam "x" $ refE "x" fzero
    typ :: AST Zero Name Star () Term
    typ = pi "a" star $ pi "_" (refE "a" fzero) (refE "a" (fsucc fzero))

kComb :: AST Zero Name Star () Elim
kComb = expr ::: typ
  where
    expr :: AST Zero Name Star () Term
...
skk = expr ::: typ
  where
    ...
    expr =
      ...
      Embed (lift iComb :@: a)
      where
        a :: AST (Succ Zero) Name Star () Term
        a = refE "a" fzero
    typ = pi "a" star $ pi "_" (refE "a" fzero) $ refE "a" (fsucc fzero)

test :: AST n Name Star () s -> IO ()
